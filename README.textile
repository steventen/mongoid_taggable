...
Fork from http://wilkerlucio.github.com/mongoid_taggable/

Key Change is: added one more taggable field called: list. It's beed used in the situation where more than one taggable fields needed.
...
bc. gem 'mongoid_taggable', :git => "git://github.com/steventen/mongoid_taggable.git", :require => "mongoid_taggable"
...
p. Tagged models get a scope called @tagged_with@, @tagged_with_all@, @tagged_with_any@ and @tagged_exclude@:
...
MyModel.tagged_with_any(['foo', 'bar'])
MyModel.tagged_exclude(['foo', 'bar'])

p. Similarly, with list model, you can do:

bc.. MyModel.listed_as('foo')
MyModel.published.listed_as('foo').count
MyModel.listed_as_all('foo', 'bar')
MyModel.listed_as_all(['foo', 'bar'])
MyModel.listed_as_any('foo', 'bar')
MyModel.listed_as_any(['foo', 'bar'])
MyModel.listed_exclude(['foo', 'bar'])
